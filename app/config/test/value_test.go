...
var _ = Describe("動作情報取得の品質検証\n", Ordered, func() {
	...
	It("動作情報の内容を確認する", func() {
		...
		Expect(env.conf.Server.Paths.CategoryList).To(Equal("/category/list"))
		Expect(env.conf.Server.Paths.ProductKeyword).To(Equal("/product/keyword/%s"))
		Expect(env.conf.Server.Paths.ProductRegister).To(Equal("/product/register"))
		Expect(env.conf.App.Hostname).To(Equal("localhost"))
		Expect(env.conf.App.Port).To(Equal(8081))
